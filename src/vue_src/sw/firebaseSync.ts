...
function b64LooksLikeZip(b64: string): boolean {
  try {
    const sample = String(b64 || '').slice(0, 32);
    if (!sample) return false;
    const padded = sample + '='.repeat((4 - (sample.length % 4)) % 4);
    const head = atob(padded);
    // PK\x03\x04
    return head.length >= 4 && head.charCodeAt(0) === 0x50 && head.charCodeAt(1) === 0x4b && head.charCodeAt(2) === 0x03 && head.charCodeAt(3) === 0x04;
  } catch (_) {
    return false;
  }
}
...
function normalizeUploadContentType({
  ...
  // Tailored resumes are always PDFs (generated by us)
  if (kind === 'tailoredResume') {
    return b64LooksLikePdf(base64) ? { contentType: 'application/pdf', ext: 'pdf' } : null;
  }

  // PDF
  if ((mt === 'application/pdf' || name.endsWith('.pdf') || !mt) && b64LooksLikePdf(base64)) {
    return { contentType: 'application/pdf', ext: 'pdf' };
  }

  // DOCX
  if ((mt === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || name.endsWith('.docx')) && b64LooksLikeZip(base64)) {
    return { contentType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', ext: 'docx' };
  }

  // TXT
  if (mt === 'text/plain' || name.endsWith('.txt')) {
...
async function maybeUploadChangedFile(
  localKey: string,
  nameKey: string,
  mimeKey: string | null,
  cloudMetaKey: string,
  kind: 'resume' | 'coverLetter' | 'tailoredResume'
) {
  if (!authState) return;
  if (!requireFirebaseConfig()) return;
  ...
  const keys = [localKey, nameKey];
  if (mimeKey) keys.push(mimeKey);

  const got = (await chrome.storage.local.get(keys)) as any;
  const b64 = String(got?.[localKey] || '').trim();
  const filename = String(got?.[nameKey] || '').trim();
  const mimeType = mimeKey ? String(got?.[mimeKey] || '').trim() : '';
  ...
  const norm = normalizeUploadContentType({ filename, mimeType, base64: b64, kind });
  if (!norm) {
    console.warn('FirebaseSync: skipping upload (unsupported or failed validation)', { localKey, filename, mimeType, kind });
    return;
  }

  const safeName = filename || `${kind}.${norm.ext}`;
  const path = `data/uploads/${uid}/${kind}/${Date.now()}_${safeName}`.replace(/\s+/g, '_');

  // Queue storage upload so it works offline too.
  const meta = await storageUpload({ path, base64: b64, contentType: norm.contentType }).catch(async (e) => {
    await enqueue({
      kind: 'storageUpload',
      payload: {
        path,
...
export function initFirebaseExtensionSync() {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (areaName === 'local') {
      ...
      // Uploads: base64 changes trigger Storage uploads; metadata is synced via profile/current immediately.
      if (changes?.Resume || changes?.Resume_name || changes?.Resume_mimeType) {
        maybeUploadChangedFile('Resume', 'Resume_name', 'Resume_mimeType', 'uploads_resume', 'resume').catch(() => {});
      }
      if (changes?.['Cover Letter'] || changes?.['Cover Letter_name'] || changes?.['Cover Letter_mimeType']) {
        maybeUploadChangedFile('Cover Letter', 'Cover Letter_name', 'Cover Letter_mimeType', 'uploads_coverLetter', 'coverLetter').catch(() => {});
      }
      if (changes?.Resume_tailored_pdf || changes?.Resume_tailored_name) {
        maybeUploadChangedFile('Resume_tailored_pdf', 'Resume_tailored_name', null, 'uploads_tailored_resume', 'tailoredResume').catch(() => {});
      }
    }
  });
